...
router.post('/login', async (req, res) => {
  ...
  if (!token) {
    return res
      .status(400)
      .send({ status: 'error', error: 'Google access token required.' })
  }
  ...
  try {
    ...
    if (response.error != undefined || response.error != null) {
      return res
        .status(400)
        .send({ status: 'error', error: 'Invalid google access token' })
    }
    ...
    if (response.data.audience != process.env.GOOGLE_CLIENT_ID) {
      return res
        .status(400)
        .send({ status: 'error', error: 'Invalid gogole access token [#2]' })
    }
    // Generate JWT token
    ...
    if (lscs.data.state == 'absent')
      return res
        .status(400)
        .send({ status: 'error', error: 'Not an LSCS member' })
